...
<head>
    ...
    <title>Rekap Riwayat Pembayaran Infaq</title>
    <style>
        body {
...
<body>
    ...
    <table class="info-table">
        ...
        <tr>
            ...
            <td>Total Riwayat</td>
            <td>: {{ count($riwayatInfaq) }} Pembayaran</td>
        </tr>
        <tr>
            <td>Paket Infaq</td>
            <td>: {{ $siswa->infaqGedung->paket ?? '-' }}</td>
            <td>Total Tagihan</td>
            <td>: Rp {{ number_format($totalTagihan, 0, ',', '.') }}</td>
        </tr>
        {{-- <tr>
            <td>Tanggal Cetak</td>
            <td colspan="3">: {{ \Carbon\Carbon::now()->format('d F Y') }}</td>
        </tr> --}}
    </table>

    <p style="text-align: center; margin: 10px 0;">Daftar Riwayat Pembayaran Infaq Gedung:</p>
    ...
    <table class="detail-table">
        ...
        <tbody>
            ...
            @if(count($riwayatInfaq) > 0)
                @foreach($riwayatInfaq as $infaq)
                    @php
                        $tanggalBayar = $infaq->tgl_bayar ?? $infaq->created_at;
                    @endphp

                    <tr>
                        <td>{{ $counter++ }}</td>
                        <td>{{ $infaq->angsuran_ke }}</td>
                        <td>Rp {{ number_format($infaq->jumlah_bayar ?? 0, 0, ',', '.') }}</td>
                        <td>
                            @if($infaq->kembalian > 0)
                                <span class="text-success">Rp {{ number_format($infaq->kembalian, 0, ',', '.') }}</span>
                            @else
                                Rp 0
                            @endif
                        </td>
                        ...
                        <td>{{ $infaq->petugas->name ?? 'Admin' }}</td>
                    </tr>
                @endforeach
                ...
                <tr style="background: #e8f4fd;">
                    <td colspan="2" style="text-align:center;">TAGIHAN</td>
                    <td colspan="2">Rp {{ number_format($totalTagihan, 0, ',', '.') }}</td>
                    <td colspan="3"></td>
                </tr>
                <tr style="background: #e8f4fd;">
                    <td colspan="2" style="text-align:center;">SISA</td>
                    <td colspan="2">
                        @if($sisaPembayaran > 0)
                            <span class="status-belum">Rp {{ number_format($sisaPembayaran, 0, ',', '.') }}</span>
                        @else
                            <span class="status-lunas">Rp 0 (LUNAS)</span>
                        @endif
                    </td>
                    <td colspan="3"></td>
                </tr>
            @else
                <tr>
                    <td colspan="7" style="text-align: center;">Tidak ada data pembayaran infaq</td>
                </tr>
            @endif
        </tbody>
    </table>
