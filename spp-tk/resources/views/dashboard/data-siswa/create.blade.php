                ...
                <div class="card-body">
                    ...
                    <form method="post" action="{{ url('dashboard/data-siswa') }}">
                            ...
                            <select name="id_kelas" class="custom-select @error('id_kelas') is-invalid @enderror" {{ count($kelas) == 0 ? 'disabled' : '' }}>
                                ...
                                @else
                                    ...
                                    @foreach($kelas as $value)
                                        <option value="{{ $value->id }}" {{ old('id_kelas') == $value->id ? 'selected' : '' }}>
                                            {{ $value->nama_kelas }}
                                        </option>
                                    @endforeach
                                @endif
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('nomor_telp') is-invalid @enderror" name="nomor_telp" value="{{ old('nomor_telp') }}">
...
<div class="input-group mb-3">
    ...
    <select name="id_infaq_gedung" class="custom-select">
        ...
        @foreach($infaq as $item)
            <option value="{{ $item->id }}" {{ old('id_infaq_gedung') == $item->id ? 'selected' : '' }}>
                Paket {{ $item->paket }} - 
