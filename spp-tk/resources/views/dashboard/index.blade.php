...
<div class="row mb-4">
                            ...
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <small>{{ $data['total_students'] - $data['unpaid_count'] }} dari {{ $data['total_students'] }} siswa</small>
                                    </div>
                                    <div class="bg-{{ $data['payment_rate'] >= 80 ? 'success' : ($data['payment_rate'] >= 50 ? 'warning' : 'danger') }} rounded p-3">
                                ...
                                <div class="progress mt-3">
                                    <div class="progress-bar bg-{{ $data['payment_rate'] >= 80 ? 'success' : ($data['payment_rate'] >= 50 ? 'warning' : 'danger') }}" 
                                         ...
                                         aria-valuenow="{{ $data['payment_rate'] }}" 
                                         aria-valuemin="0" aria-valuemax="100"></div>
                                </div>
                                <div class="mt-2">
                                    <a href="#" data-toggle="modal" data-target="#unpaidModal-{{ Str::slug($kelas) }}" 
                                       class="text-danger">
    ...
    @foreach($pemasukanSPPPerKelas as $kelas => $data)
    <div class="modal fade" id="unpaidModal-{{ \Illuminate\Support\Str::slug($kelas) }}" tabindex="-1" role="dialog" aria-hidden="true">
                ...
                datasets: [
                    {
                        ...
                        data: {!! json_encode(array_map(function($item) { return $item['previous']; }, $pemasukanSPPPerKelas)) !!},
                    ...
                    },
                    {
                        ...
                        data: {!! json_encode(array_map(function($item) { return $item['current']; }, $pemasukanSPPPerKelas)) !!},
