            ...
            <form method="post" action="{{ route('entry-pembayaran.update', $edit->id) }}">
               ...
               <div class="form-group">
                  ...
                  <input type="text" class="form-control @error('nisn') is-invalid @enderror" 
                         name="nisn" value="{{ old('nisn', $edit->siswa->nisn) }}" readonly>
               ...
               <div class="form-group">
                  ...
                  <select class="form-control @error('bulan') is-invalid @enderror" name="bulan">
                     ...
                     <option value="{{ $bulan }}" {{ old('bulan', $edit->bulan) == $bulan ? 'selected' : '' }}>
                        {{ ucfirst($bulan) }}
                     </option>
                     @endforeach
                  </select>
               ...
               <div class="form-group">
                  ...
                  <input type="number" class="form-control @error('jumlah_bayar') is-invalid @enderror" 
                         ...
                         min="{{ $edit->nominal_spp + ($edit->nominal_konsumsi ?? 0) + ($edit->nominal_fullday ?? 0) }}" required>
               ...
               <div class="form-group">
                  ...
                  <select class="form-control @error('is_lunas') is-invalid @enderror" name="is_lunas">
                     <option value="1" {{ old('is_lunas', $edit->is_lunas) ? 'selected' : '' }}>Lunas</option>
                     <option value="0" {{ !old('is_lunas', $edit->is_lunas) ? 'selected' : '' }}>Belum Lunas</option>
                  </select>
                  <span class="text-danger">@error('is_lunas') {{ $message }} @enderror</span>
               </div>
