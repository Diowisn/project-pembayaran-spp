                        ...
                        <tbody>
                            ...
                            @foreach ($spp as $value)
                                <tr>
                                    ...
                                    <td>
                                        <strong>SPP:</strong> Rp
                                        {{ number_format($value->nominal_spp, 0, ',', '.') }}<br>
                                        @if ($value->nominal_konsumsi)
                                            <strong>Konsumsi:</strong> Rp
                                            {{ number_format($value->nominal_konsumsi, 0, ',', '.') }}<br>
                                        @endif
                                        @if ($value->nominal_fullday)
                                        ...
                                        <div class="hide-menu">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="actiondd">
                                                <a class="dropdown-item"
                                                    href="{{ url('dashboard/data-spp/' . $value->id . '/edit') }}">
                                                    <i class="ti-pencil"></i> Edit
                                                </a>
                                                <form method="post"
                                                    action="{{ url('dashboard/data-spp', $value->id) }}"
                                                    id="delete{{ $value->id }}">
                                                    @csrf
                                                    @method('delete')
                ...
                @if ($spp->lastPage() != 1)
                    <div class="btn-group float-right">
                        <a href="{{ $spp->previousPageUrl() }}"
                            class="btn btn-success {{ $spp->currentPage() == 1 ? 'disabled' : '' }}">
                        ...
                        <a href="{{ $spp->nextPageUrl() }}"
                            class="btn btn-success {{ $spp->currentPage() == $spp->lastPage() ? 'disabled' : '' }}">
